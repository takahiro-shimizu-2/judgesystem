...
- planning 時に必ず GitNexus runtime artifact を生成し、task ごとの GitNexus binding を作る
- Issue 分解の前に Omega の `Intent / Strategic Plan` artifact を生成し、その後に DAG・execution plan・execution report を生成する
- 実行後は Omega `Deliverable / Learning` artifact も生成し、次回 run へ learning context を carry-forward できる
...
- intent / strategic plan / deliverable / learning / execution plan / execution report が生成される
- GitNexus runtime artifact と task binding が生成される
