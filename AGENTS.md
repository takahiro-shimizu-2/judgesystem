...
Deploy は approval / build / preflight / healthcheck / rollback command を明示した contract がある場合だけそこまで実行し、無い能力は成功扱いにしないこと。protected deploy は `autonomous-deploy-execute.yml` の GitHub Environment approval 経路を優先すること。
