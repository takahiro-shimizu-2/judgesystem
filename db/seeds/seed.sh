...
load_tsv() {
  ...
  echo "\\COPY ${table_spec} FROM '${file}' WITH (FORMAT csv, DELIMITER E'\\t', HEADER true, NULL '')" >> "$SCRIPT"
}

# --- Master data tables ---

load_tsv "$MASTER_DIR/company_master.txt" \
...
load_tsv "$MASTER_DIR/office_master.txt" \
...
load_tsv "$MASTER_DIR/partners_master.txt" \
...
load_tsv "$MASTER_DIR/announcements_documents_master.txt" \
...
load_tsv "$MASTER_DIR/construction_master.txt" \
...
load_tsv "$MASTER_DIR/technician_qualification_master.txt" \
...
load_tsv "$MASTER_DIR/office_registration_authorization_master.txt" \
