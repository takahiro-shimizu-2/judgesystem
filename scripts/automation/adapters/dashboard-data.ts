...
export async function runDashboardDataCli() {
  try {
    ...
    if (token) {
      ...
      agents = buildAgentData(repoMetrics.skillBus);
...
function buildAgentData(skillBus: SkillBusSummary) {
  const groups = new Map<string, { completed: number; scoreTotal: number }>();

  for (const run of skillBus.recentRuns) {
