...
export class ProjectsV2Client {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      if (!result.repository.projectV2) {
        throw new Error(
          `Project V2 #${this.config.projectNumber} was not found for ${this.config.owner}/${this.config.repo}.`,
        );
      }
  ...
  async updateStatus(itemId: string, status: string): Promise<void> {
    ...
    if (!statusOption) {
      throw new Error(
        `Status option '${status}' not found. Available: ${statusField.options
  ...
  private async getContentNodeId(contentType: 'issue' | 'pull_request', number: number): Promise<string> {
    ...
    const contentNode =
      contentKey === 'issue' ? result.repository.issue : result.repository.pullRequest;
    if (!contentNode?.id) {
