...
const SORT_FIELDS = [
  ...
  { field: 'status', label: '参加可否', ascLabel: '可→不可', descLabel: '不可→可' },
  { field: 'workStatus', label: '着手状況', ascLabel: '未着手→完了', descLabel: '完了→未着手' },
  { field: 'priority', label: '優先度', ascLabel: '高い', descLabel: '低い' },
  { field: 'prefecture', label: '都道府県', ascLabel: '北→南', descLabel: '南→北' },
  { field: 'evaluatedAt', label: '判定日', ascLabel: '古い', descLabel: '新しい' },
] as const;
...
// フィルタータブの定義
const FILTER_TABS = [
  { id: 'status', label: '参加可否', icon: <CheckCircleIcon sx={iconStyles.small} /> },
  { id: 'priority', label: '優先度', icon: <StarIcon sx={iconStyles.small} /> },
  { id: 'workStatus', label: '着手状況', icon: <WorkStatusIcon sx={iconStyles.small} /> },
  { id: 'bidType', label: '入札方式', icon: <BidTypeIcon sx={iconStyles.small} /> },
...
interface DisplayConditionsPanelProps {
  searchQuery: string;
...
export function DisplayConditionsPanel({
  searchQuery,
  onSearchChange,
  sortModel,
  onSortModelChange,
  ...
  const toggleWorkStatus = (status: WorkStatus) => {
    const newWorkStatuses = filters.workStatuses.includes(status)
      ? filters.workStatuses.filter(s => s !== status)
      : [...filters.workStatuses, status];
    onFilterChange({ ...filters, workStatuses: newWorkStatuses });
  };
            ...
            endAdornment: searchQuery && (
              <InputAdornment position="end">
                <IconButton
                  ...
                  sx={rightPanelStyles.searchIcon}
                >
